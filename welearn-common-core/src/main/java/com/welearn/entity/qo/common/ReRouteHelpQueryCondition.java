package com.welearn.entity.qo.common;

import com.welearn.entity.po.common.ReRouteHelp;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.NoArgsConstructor;
import io.swagger.annotations.ApiModel;

/**
 * Description : ReRouteHelp Query Condition
 * Created by Setsuna Jin.
 * @author Setsuna Jin Generate By CodeSmith 7.0 At 2019/3/4 16:18:22
 * @see com.welearn.entity.po.common.ReRouteHelp
 */
@Data
@EqualsAndHashCode(callSuper = true)
@NoArgsConstructor
@ApiModel(value = "ReRouteHelpQueryCondition", description = "ReRouteHelp 查询条件")
public class ReRouteHelpQueryCondition extends ReRouteHelp {

}
